...
class model_structure_loader(object):
	...
	def lstm_model(self, seq_len=30, batch_size=None, stateful=True):
		...
		def one_hot(room_id_bit):
			res = tf.keras.backend.one_hot(room_id_bit, num_classes=self.EMBEDDING_DIM)
			return res
		'Work around'
		one_hot_embedding_id = tf.keras.layers.Lambda(one_hot)(room_id_bit)
		# one_hot_embedding_id = tf.keras.backend.one_hot(room_id_bit, num_classes=self.EMBEDDING_DIM)

		encoder_input = tf.keras.Input(name='Encoder_input', shape=(self.context_vector_length,), batch_size=batch_size,
		                               dtype=tf.int32)
		...
		predicted_char_layer = tf.keras.layers.TimeDistributed(
			tf.keras.layers.Dense(self.dict_length, activation='softmax',
			                      kernel_regularizer=tf.keras.regularizers.l2(self.regularizer_coefficient)))
		...
		encoder_output_in = tf.keras.Input(shape=(self.context_vector_length + 2, self.EMBEDDING_DIM,))
		decode_embedding = embedding_layer(decoder_inputs)
